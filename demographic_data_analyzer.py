...
def calculate_demographic_data(print_data=True):
    ...
    rich_by_country_percentage = (rich_by_country/ total_by_country*100).fillna(0)

    highest_earning_country = rich_by_country_percentage.idxmax()
    highest_earning_country_percentage = round(rich_by_country_percentage.max(),1)

    # Identify the most popular occupation for those who earn >50K in India.
    top_IN_occupation_series = df[(df['native-country'] == 'India')& (df['salary'] == '>50K')]['occupation'].mode()

    top_IN_occupation = top_IN_occupation_series[0] if not top_IN_occupation_series.empty else "Unknown"
